...
\begin{figure}[H]
    ...
    \caption{Power-law validation: cumulative degree distribution (log-log scale). The approximately linear trend in log-log space is consistent with a power-law distribution, a hallmark of scale-free networks\cite{clauset2009powerlaw}. Concentration analysis shows top 1\% of artists control 23.5\%, top 5\% control 46.5\%, and top 10\% control 58.2\% of all sampling events.}
...
\begin{figure}[H]
    ...
    \caption{Hub analysis scatter plot. X-axis: out-degree (samples used). Y-axis: in-degree (times sampled). Top 15 most-sampled artists are highlighted in red. Four quadrants are delineated by median lines: Authorities (top-left), Bridges (top-right), Heavy Samplers (bottom-right), and Peripheral Artists (bottom-left).}
...
\begin{itemize}
    ...
    \item \textbf{Bridges} (top-right quadrant): Artists who both sample heavily AND get sampled. As discovered in our bridge analysis (Figure~\ref{fig:topbridges}), evolutionary nodes like \textbf{JAY-Z}, \textbf{Beastie Boys}, and \textbf{Ye (Kanye West)} maintain a near-perfect balance, acting as the crucial connective tissue of modern music.
    ...
    \item \textbf{Peripheral Artists} (bottom-left quadrant): Artists with below-median in-degree and out-degree. These represent the majority of nodes in the long tail of the distribution --- niche creators or artists with few documented sampling relationships in the dataset.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fig9_top_bridges.pdf}
    \caption{Top 15 Evolutionary Bridges. These artists maintain the highest geometric mean of in-degree and out-degree, representing key evolutionary nodes in the musical network.}
    \label{fig:topbridges}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{fig10_authority_composition.pdf}
    \caption{Authority Context: Internal vs External Influence. This chart reveals whether an artist's authority is driven by a massive, isolated sub-culture (Internal) or by universal mainstream adoption across genres (External).}
...
To effectively explore the immense scale of the expanded dataset (over 58,000 edges), I also developed an interactive web-based D3.js visualization that allows users to pan, zoom, and click on nodes to inspect real-time Authority Scores and isolate specific sampling lineages. The interactive visualization is available at: \href{file:../project/outputs/interactive_genealogy.html}{interactive\_genealogy.html}
...
\subsubsection{Macro-Flow: Inter-Community Genealogy}
While the Louvain algorithm identifies well-defined communities, music evolution occurs when these communities interact. By aggregating edges at the cluster level, we mapped the macroscopic flow of influence.

As shown in Figure~\ref{fig:macroflow}, distinct musical pipelines emerge. Instead of analyzing individual artists, we can observe how entire ecosystems feed into others, providing a higher-level view of genre cross-pollination.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{fig11_macro_community_flow.pdf}
    \caption{Macroscopic Sampling Flow Between Communities. Node size represents total incoming cluster influence, and edge thickness represents the volume of sampling between distinct communities.}
    \label{fig:macroflow}
\end{figure}

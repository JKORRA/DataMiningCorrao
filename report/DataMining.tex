...
This filtering reduced the edge count from 22,135 to 20,728 edges at the artist level (after collapsing multiple song-level edges between the same artist pair). The final graph at the song level contains 58,621 sampling events—each representing a specific recording-to-recording relationship—which are then aggregated into weighted artist-level edges for the PageRank and clustering analyses. This distinction is important: the 58,621 figure reflects fine-grained song-level interactions, while the 20,728 figure represents the collapsed artist-to-artist topology used for structural analysis. Both levels of granularity are preserved in the processed dataset.
...
The final processed graph, after extensive data engineering using MusicBrainz GIDs and removing self-loops (artists sampling themselves), comprises \textbf{58,621 sampling events} (edges) connecting \textbf{68,527 songs} (nodes) across \textbf{23,503 unique artists}.
...
\begin{figure}[H]
    ...
    \caption{Power-law validation: cumulative degree distribution (log-log scale). The approximately linear trend in log-log space is consistent with a power-law distribution, a hallmark of scale-free networks. Concentration analysis shows top 1\% of artists control 23.6\%, top 5\% control 46.6\%, and top 10\% control 58.2\% of all sampling events.}
...
To assess the robustness of our PageRank authority rankings, we performed external validation using a curated list of the 10 most-sampled artists from \textbf{WhoSampled}, an independent sampling reference. We computed the Spearman rank correlation between the WhoSampled ground-truth ordering and our PageRank rankings, considering only the same 10-artist subset.

The resulting Spearman correlation coefficient is $\rho = 0.37$, indicating a \textbf{moderate positive correlation}. While not perfect, this is notable given fundamental methodological differences: WhoSampled ranks by raw sampling count, whereas PageRank measures structural influence. The moderate agreement confirms that our authority metric captures genuine influence while diverging in meaningful ways---for example, PageRank elevates structurally influential artists like Lyn Collins (ground-truth rank 5, system rank 2) and Kraftwerk (ground-truth rank 6, system rank 3) above high-volume but less structurally central artists like Madonna (ground-truth rank 10, system rank 9) and Public Enemy (ground-truth rank 2, system rank 10).

Full match results are saved in \texttt{outputs/external\_validation.csv}. This validation provides independent confirmation that our PageRank implementation identifies genuinely influential artists while offering a dimension of authority distinct from raw count-based metrics.
...
The Label Propagation Algorithm detected \textbf{13,855 distinct clusters} with an intra-cluster edge fraction (simplified modularity) of \textbf{0.1926}, indicating a complex and overlapping community structure. This metric represents the proportion of edges that fall within the same cluster; a value of 0.19 means that approximately 19\% of sampling connections occur between artists in the same genealogical community, while 81\% cross community boundaries. This suggests the music network is highly interconnected across genre boundaries.
...
\begin{itemize}
    \item \textbf{Largest Cluster}: Contains 675 songs, representing the dominant, highly integrated core of modern sampling.
    \item \textbf{Simplified Modularity 0.1926}: 19\% of edges are intra-cluster, indicating a low but detectable community structure. This is expected in modern digital music where producers freely mix samples from disparate sources.
    \item \textbf{13,855 clusters}: The vast number of clusters highlights the fragmented nature of niche internet subcultures and isolated sampling chains that haven't connected to the main network.
\end{itemize}

\subsubsection{Macro-Flow: Inter-Community Genealogy}
While the Label Propagation Algorithm identified isolated communities, music evolution occurs when these communities interact. By aggregating edges at the cluster level, we mapped the macroscopic flow of influence.
...
\begin{itemize}
    ...
    \item \textbf{13,855 genealogical families} detected with an intra-cluster edge fraction of 0.1926, indicating a highly interconnected and overlapping modern music landscape where 81\% of sampling crosses community boundaries.
